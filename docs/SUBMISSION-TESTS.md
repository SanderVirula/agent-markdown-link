...
4. **Prompt:** "Capture our decision to keep canonical summaries derived." **Expected:** explicit capture using `sourceHost`, kind, title, proposed knowledge, and rationale. **Result:** one new immutable memory record that is immediately searchable, with no canonical-summary edit.
...
2. **Scenario:** the user asks for content outside configured search roots and the shared automatic-memory folder. **Expected fallback:** do not return that content; explain that recall is limited to configured roots. **Why:** broader reads would violate the explicit local scope.
3. **Scenario:** a capture is malformed, oversized, or contains a recognized credential. **Expected fallback:** return only the sanitized error and create no memory record. **Why:** bounded input and secret refusal protect the local vault.
